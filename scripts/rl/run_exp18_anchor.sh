...
# Best run : run_007 (ancre complète : demo_buffer=true & bc_loss=true, seed 41,
# gap_best = 4.8% — meilleur des bras anti-washout WS-1c, batterie 100 kW).
CONFIG="configs/sweeps/exp18_anchor/run_007.yaml"
MODEL="results/exp18_anchor/run_007/best_model.zip"
...
if [ ! -f "$MODEL" ]; then
    ...
    echo "Lancez d'abord le sweep : micromamba run -n stageCorse python experiments/run_sweep.py --sweep configs/sweeps/exp18_anchor/"
    exit 1
fi
...
for c in "${CASES[@]}"; do
    ...
    OUT="monitoring/runs/rl_exp18_anchor_${c}_monitoring_table.csv"
    if [ ! -f "$PV" ] || [ ! -f "$LOAD" ]; then
