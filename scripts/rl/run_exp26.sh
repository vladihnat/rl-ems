...
# Rejoue un modèle RL du sweep exp26 (PBRS au prix dual MILP (store_value_mode=milp_dual)) sur la
# fenêtre overfit (train==eval, 4 jours). But : valider que le potentiel λ(t) exact supprime le dump t0 et
# capturent l'heure de pointe export 21-22h (cf. audit exp23). Pendant MILP : scripts/milp/run_exp26.sh.
#
# Usage : ./scripts/rl/run_exp26.sh [SWEEP] [RUN] [extra python args...]
#   SWEEP : exp26_hiver_haute (défaut) | exp26_ete_haute  (ou forme courte hiver_haute | ete_haute)
#   RUN   : run_000 (défaut) | run_NNN

_SWEEP_ARG="${1:-${SWEEP:-exp26_hiver_haute}}"
if [ "$#" -gt 0 ]; then shift; fi
case "$_SWEEP_ARG" in
    exp26_*) SWEEP="$_SWEEP_ARG" ;;
    *)       SWEEP="exp26_${_SWEEP_ARG}" ;;
esac
...
echo "[sim] exp26 RL | sweep=$SWEEP | run=$RUN | PV=$PV | load=$LOAD -> $OUT"
