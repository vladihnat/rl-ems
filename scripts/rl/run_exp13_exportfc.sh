...
MODEL="results/exp13_exportfc/run_002/best_model.zip"
...
if [ ! -f "$MODEL" ]; then
    ...
    echo "Lancez d'abord le sweep : micromamba run -n stageCorse python experiments/run_sweep.py --sweep configs/sweeps/exp13_exportfc/"
    exit 1
fi
...
for c in "${CASES[@]}"; do
    ...
    OUT="monitoring/runs/rl_exp13_exportfc_${c}_monitoring_table.csv"
    if [ ! -f "$PV" ] || [ ! -f "$LOAD" ]; then
