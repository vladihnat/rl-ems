...
# Best run : run_002 (gap_best = 14.9%)
CONFIG="configs/sweeps/exp17_bc/run_002.yaml"
MODEL="results/exp17_bc/run_002/best_model.zip"
...
if [ ! -f "$MODEL" ]; then
    ...
    echo "Lancez d'abord le sweep : micromamba run -n stageCorse python experiments/run_sweep.py --sweep configs/sweeps/exp17_bc/"
    exit 1
fi
...
for c in "${CASES[@]}"; do
    ...
    OUT="monitoring/runs/rl_exp17_bc_${c}_monitoring_table.csv"
    if [ ! -f "$PV" ] || [ ! -f "$LOAD" ]; then
