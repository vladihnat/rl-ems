...
# Rejoue un modèle RL du check exp24 (relâchement no-grid-charging, milpIncoherences.md #5) sur la
# fenêtre overfit (train==eval, 4 jours, prix fixes 0.15/0.04). Pendant MILP : scripts/milp/run_exp24.sh.
#
# exp24 ne varie QUE battery.pv_charge_mode : run_000=surplus (strict), run_001=total (relâché).
#
# Usage : ./scripts/rl/run_exp24.sh [SWEEP] [RUN] [extra python args...]
#   SWEEP : exp24_hiver_haute (défaut) | exp24_ete_haute
#           ou forme courte : hiver_haute | ete_haute (le préfixe exp24_ est ajouté auto)
#   RUN   : run_000 (surplus, défaut) | run_001 (total)
#
# Env vars surchargeables : SWEEP=exp24_ete_haute ./scripts/rl/run_exp24.sh run_001

# 1er arg : SWEEP (forme longue ou courte) — défaut hiver_haute
_SWEEP_ARG="${1:-${SWEEP:-exp24_hiver_haute}}"
if [ "$#" -gt 0 ]; then shift; fi
# Normalisation : accepter la forme courte (hiver_haute ou ete_haute)
case "$_SWEEP_ARG" in
    exp24_*) SWEEP="$_SWEEP_ARG" ;;
    *)       SWEEP="exp24_${_SWEEP_ARG}" ;;
esac
...
# Dériver le cas de simulation depuis le nom du sweep (exp24_hiver_haute → hiver_haute)
CASE="${SWEEP#exp24_}"
WINDOW="${CASE}_4d"
CONFIG="configs/sweeps/${SWEEP}/${RUN}.yaml"     # config PAR RUN (pv_charge_mode varie → replay fidèle)
...
echo "[sim] exp24 RL | sweep=$SWEEP | run=$RUN | PV=$PV | load=$LOAD -> $OUT"
