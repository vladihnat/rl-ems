...
# Référence MILP du run_001 d'exp19 (PBRS sigma_store=1.0, gap_best = 3.9%).
# Le PBRS est invariant sur l'optimum → le MILP est identique à tous les runs
# store ; on prend le config du run RL retenu pour une comparaison cohérente
# (batterie 100 kW, même scénario que le RL).
...
for c in "${CASES[@]}"; do
    ...
    OUT="monitoring/runs/milp_exp19_store_${c}_monitoring_table.csv"
    PLAN_OUT="monitoring/runs/milp_exp19_store_${c}_plan.csv"
    if [ ! -f "$PV" ] || [ ! -f "$LOAD" ]; then
    ...
    # --score-days 1 : fenêtres de 2 jours → coût scoré sur le jour 1 seul. Le MILP
    # résout l'horizon complet (2 j) mais on exclut son dump de batterie de fin
    # d'horizon (jour 2), irréaliste en déploiement, du calcul de coût comparé.
