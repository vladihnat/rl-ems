...
# MILP est donc identique à exp25/exp27 : mêmes données/prix, on réutilise configs/overfit/exp21_overfit_
...
for f in "$CONFIG" "$PV" "$LOAD"; do
    if [ ! -f "$f" ]; then
        ...
        echo "Config attendu : configs/overfit/exp21_overfit_${CASE}.yaml (prix variables, partagé avec exp25/exp27)."
