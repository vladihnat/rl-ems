...
# par période suffit. On réutilise configs/overfit/exp22_${CASE}.yaml (batterie/grille/PV/charge identiques
...
WINDOW="${CASE}_4d"
CONFIG="configs/overfit/exp22_${CASE}.yaml"
...
for f in "$CONFIG" "$PV" "$LOAD"; do
    if [ ! -f "$f" ]; then
        ...
        echo "Config attendu : configs/overfit/exp22_${CASE}.yaml (partagé avec le sweep exp22)."
